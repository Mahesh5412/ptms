...
class ListItem extends React.Component {
  render() {
    ...
    // Alert.alert("taskStatus"+item.status);
    return (
          ...
          <CollapseBody>
            ...
            <View style={styles.box1}>
              ...
              {/* <View styles={{ paddingRight: 40,}}> */}
                <Text style={styles.signUpText002} >Status:{item.taskStatus}% completed </Text>
              {/* </View> */}
...
export default class Pending extends Component {
  ...
  componentDidMount() {
    //Getting the employee pending maintask list
    log("Debug", "user Manageemployee view pending tasks screen is loaded");
    this.getEmployeesPendingTaskList()
  }
  //Refresh the employee pending maintask list
  onRefresh() {
    this.getEmployeesPendingTaskList();
  }
  ...
  TaskChat(item, index) {
    log("Info", "userManageEmployeeTaskPending:TaskChat(item, index) method is used to naviagate to chat");
  ...
  getEmployeesPendingTaskList() {
    log("Info", "UserManageEmployeeTaskPending:getEmployeesPendingTaskList() used to get employee pending tasks");
              ...
              fetch(API + 'getSubtasks.php',
                ...
                .then((responseJson) => {
                  ...
                  if (responseJson.status === 'True') {
                    ...
                    log("Info", "no  employee pending tasks at user side");
                    this.setState({
                      isLoading: false,
                    })
                    Snackbar.show({
                ...
                .catch((error) => {
                  console.error(error);
                  log("Error", "Error in getting employee pending tasks at user side");
                });
            }
          });
        });
